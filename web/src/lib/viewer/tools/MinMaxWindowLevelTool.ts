...
import type { EventTypes } from '@cornerstonejs/tools/src/types';
 ...
 *
 */
class MinMaxWindowLevelTool extends BaseTool {
  ...
  mouseDragCallback(evt: EventTypes.InteractionEventType) {
    ...
    if (!enabledElement) throw new Error('no enabled element');
    ...
    if (viewport instanceof VolumeViewport) {
      ...
      if (enabledElement === undefined) throw new Error('no targetId');
  ...
  getNewRange({ viewport, deltaPointsCanvas, volumeId, lower, upper }) {
    ...
    const lDelta = deltaPointsCanvas[0] * multiplier;
    const uDelta = deltaPointsCanvas[1] * multiplier;
...
MinMaxWindowLevelTool.toolName = 'MinMaxWindowLevel';
export default MinMaxWindowLevelTool;
